...
## Interactions
comment_feed (admin, fleet_manager, operator — NOT viewer) gates comments, replies and ALL reactions. Two closed vocabularies, deliberately different: items take FeedItem::REACTIONS (👍 seen / ✅ done / ⚠️ attention), comments take FeedComment::REACTIONS (👍 like / ✅ acknowledge / ❌ reject) — ⚠️ on a comment or ❌ on an item is refused. Both are DB-unique toggles per (target,user,emoji). Replies are ONE level deep by design: addReply flattens a reply-to-a-reply onto the root comment (parent_id always points at a root) so threads stay readable on a phone. Deletion: author or pin_feed holder; deleting a root cascades its replies (FK). Livewire computed props (getXProperty) are a Blade affordance — in PHP call the getter, $this->x is UndefinedThisPropertyFetch to psalm. Livewire test harness: after an action 403s, the component snapshot is spent (second call() throws Invalid snapshot) — use a fresh Livewire::test; findOrFail inside an action surfaces as ModelNotFoundException, not an HTTP 404.
