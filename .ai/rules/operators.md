...
## Compliance alerts are idempotent by dedupe key, never by 'did we run today'
ComplianceAlertService (scheduled daily 05:30 via operators:check-compliance) alerts admins+fleet_managers at the config/operators.php windows (30 mandatory, 14/7 configurable escalations) and once on expiry. Idempotency lives in the notification row itself: data->dedupe_key = "{kind}:{credential_id}:{milestone}", checked before dispatch — so re-runs, missed days, and edited warning windows can never repeat an alert, and only the NEAREST applicable milestone fires (a credential first seen 5 days out gets one 7-day alert, not a 30+14+7 backlog). New alert surfaces must reuse these notification rows, not re-derive expiry themselves.

## Documents: private disk, audited controller, kind-based gating
operator_documents rows hide disk+path ($hidden) — a serialised document must never hand a client the storage location. The ONLY way a file leaves storage is OperatorDocumentController: route-model binding (team scope makes foreign ids 404), OperatorPolicy::view, viewMedical additionally when kind=medical, an activity_logs entry per download, and Content-Disposition download + nosniff so an uploaded HTML/SVG can never execute in the app's origin. Uploads: pdf/jpg/png only, 10MB, hashed filenames under operator-documents/{team}/{operator}/ on the local (private) disk; medical-kind uploads require manageMedical. Deletes are soft — the row is the audit trail.
