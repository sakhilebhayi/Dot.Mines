 ...
 * dismissed_unresolved. The manufacturer alert normaliser used to
 ...
 * Postgres. parseAlert() is protected (a shared helper, not part of
...
class ManufacturerAlertsShapeTest extends TestCase
{
    ...
    public function test_parse_alert_defaults_missing_status_to_active_not_the_legacy_new(): void
    {
        $service = new CATService(['base_url' => 'https://api.example.test']);

        $parsed = $this->invokeProtected($service, 'parseAlert', [
    ...
    public function test_parse_alert_keeps_a_status_the_manufacturer_actually_provided(): void
    {
        $service = new CATService(['base_url' => 'https://api.example.test']);
        ...
        $this->assertSame('resolved', $parsed['status']);
    }
}
