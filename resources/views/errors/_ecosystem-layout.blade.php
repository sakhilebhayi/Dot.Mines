...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="scroll-smooth">
    <head>
        ...
        <title>{{ $code }} — @yield('title') · Dot.Mines</title>
        ...
        <style nonce="{{ request()->attributes->get('csp_nonce') }}">
            ...
            * { box-sizing: border-box; }
            ...
            body { margin: 0; font-family: var(--font-body); background: var(--paper); color: #c9b896; min-height: 100dvh; display: flex; flex-direction: column; }
        ...
        <header style="background: var(--chrome);">
            <nav style="max-width: 1400px; margin: 0 auto; padding: 14px 20px; display: flex; align-items: center; justify-content: space-between;">
                <a href="/" class="press" style="display: flex; align-items: center; gap: 10px;">
                    ...
                    <img src="{{ asset($logoLightPath ?? 'images/logo.png') }}" alt="Dot.Mines" style="height: 40px; width: auto;">
                </a>
                <div style="display: flex; align-items: center; gap: 12px;">
                    @auth
                        <a href="{{ route('dashboard') }}" class="press" style="padding: 10px 20px; background: var(--accent); color: #211a14; font-family: var(--font-display); font-weight: 600; font-size: 14px; border-radius: 999px; text-decoration: none;">Dashboard</a>
                    @else
                        @if (Route::has('login'))
                    ...
                    <div style="border-top: 1px solid var(--line); padding-top: 28px; text-align: left;">
                        ...
                        <div style="display: flex; gap: 8px; overflow-x: auto; padding: 2px 2px 8px; scroll-snap-type: x proximity; -webkit-overflow-scrolling: touch;">
                            @foreach ($discover as $platform)
                                <a href="{{ $platform['url'] }}" class="press" style="flex: 0 0 auto; scroll-snap-align: start; display: flex; align-items: center; gap: 8px; padding: 7px 14px 7px 7px; background: #2c2319; border: 1px solid var(--line); border-radius: 999px; text-decoration: none; white-space: nowrap;">
